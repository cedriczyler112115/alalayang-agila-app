...
@section('title', 'Edit Region Officers - Caragados EC')
...
<div style="margin-top: 2rem;">
    ...
    <div class="card" style="padding: 2rem; margin-bottom: 2rem;">
        ...
        @endif
        <form action="{{ route('libraries.region.update', $region->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            
            <h2 style="font-size: 1.25rem; font-weight: 600; color: var(--text-main); margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border-color);">Region Identification</h2>
            ...
            <div class="form-group" style="margin-bottom: 2rem;">
                ...
                @if($region->logo)
                    <div style="margin-top: 1rem;">
                        <img src="{{ asset('storage/' . $region->logo) }}" alt="{{ $region->name }} Logo" style="max-width: 150px; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                    </div>
                @endif
            </div>
            ...
            <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;">
                @foreach($regional_positions as $position)
                    @php
                        ...
                        $assignedOfficer = $current_officers->firstWhere('lib_regional_position_id', $position->id);
                    @endphp
...
$(document).ready(function() {
    ...
    $('.officer-select').on('change', function(e) {
        ...
        fetch(`{{ route('libraries.region.assign_officer', $region->id) }}`, {
            method: 'POST',
            headers: {
