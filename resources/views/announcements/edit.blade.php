...
@section('title', 'Edit Announcement - Caragados EC')
...
<div style="margin-top: 2rem;">
    <div style="display: flex; align-items: center; margin-bottom: 2rem;">
        ...
        <div>
            <h1 style="font-size: 2rem; font-weight: 700; margin-bottom: 0.25rem; letter-spacing: -0.025em;">Edit <span style="color: var(--accent);">Announcement</span></h1>
            <p style="color: var(--text-muted); font-size: 1.05rem;">Update the details of the existing announcement.</p>
        </div>
    </div>
    ...
    <div class="card" style="padding: 2rem;">
        ...
        <form action="{{ route('announcements.update', $announcement) }}" method="POST" id="announcementForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" class="form-control" value="{{ old('title', $announcement->title) }}" placeholder="Enter a descriptive title" required maxlength="255">
            </div>
            ...
            <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1.5rem; margin-top: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="datetime-local" id="published_at" name="published_at" class="form-control" value="{{ old('published_at', $announcement->published_at ? $announcement->published_at->format('Y-m-d\TH:i') : '') }}">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control" required>
                        <option value="draft" {{ old('status', $announcement->status) === 'draft' ? 'selected' : '' }}>Save as Draft</option>
                ...
                @if(auth()->user()->lib_regional_position_id)
                    <div style="margin-bottom: 0;">
                        ...
                        <label class="form-label" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; margin-bottom: 0.75rem;">
                            <input type="checkbox" name="post_as_global" value="1" {{ old('post_as_global', $announcement->scope === 'global') ? 'checked' : '' }} class="scope-cb" style="width: 1.25rem; height: 1.25rem; accent-color: var(--accent);">
                            <span style="font-weight: 600; color: var(--text-main);">Post as Global Announcement</span>
                        </label>
                        <label class="form-label" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; margin-bottom: 0.75rem;">
                            <input type="checkbox" name="post_as_regional" value="1" {{ old('post_as_regional', $announcement->scope === 'regional') ? 'checked' : '' }} class="scope-cb" style="width: 1.25rem; height: 1.25rem; accent-color: var(--accent);">
                        ...
                        <label class="form-label" style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; margin-bottom: 0;">
                            <input type="checkbox" name="post_on_my_club" value="1" {{ old('post_on_my_club', $announcement->scope === 'club') ? 'checked' : '' }} class="scope-cb" style="width: 1.25rem; height: 1.25rem; accent-color: var(--accent);">
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid var(--border-color);">
                <button type="submit" class="btn btn-primary" id="submitBtn" style="flex: 1; padding: 0.8rem;">
                    <span class="btn-text">Update Announcement</span>
