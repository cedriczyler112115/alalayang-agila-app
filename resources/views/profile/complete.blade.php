    ...
    <div style="margin-top: 2rem;">
        <div class="card">
            ...
            <div class="card-body">
                @if($errors->any())
                    <div
                        style="background-color: rgba(239, 68, 68, 0.1); color: var(--danger); padding: 1rem; border-radius: var(--radius-md); margin-bottom: 1.5rem; font-size: 0.9rem;">
                ...
                <form action="{{ route('profile.complete') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-2" style="display: grid; grid-template-columns: 1fr; gap: 1rem;"
                        id="job-office-grid">
                        <div class="form-group">
                            <label class="form-label" for="current_job">Current Job / Occupation</label>
                            <input type="text" id="current_job" name="current_job" class="form-control"
                                value="{{ old('current_job', $user->current_job) }}">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="address" name="address" class="form-control"
                            value="{{ old('address', $user->address) }}" placeholder="City, State, Country" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Address Location on Map</label>
                        <label class="form-label" style="font-size: 0.8rem; color: red; font-style: italic;">
                            Scroll in the left or right corner to scroll down.
                        </label>
                        <div id="map"
                            style="height: 500px; width: 100%; border-radius: var(--radius-md); border: 1px solid var(--border-color); margin-bottom: 0.5rem;">
                        </div>
                        <input type="hidden" name="location" id="location-input"
                            value="{{ old('location', $user->location) }}">
                        <p style="font-size: 0.8rem; color: var(--text-muted);">Click on the map to tag your precise address
                            location.</p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="contact_number" name="contact_number" class="form-control"
                            value="{{ old('contact_number', $user->contact_number) }}" placeholder="+1234567890" required>
                    </div>

                    <div class="grid grid-cols-2">
                        <div class="form-group">
                            <label class="form-label" for="contact_person_emergency">Emergency Contact Person</label>
                            <input type="text" id="contact_person_emergency" name="contact_person_emergency"
                                class="form-control"
                                value="{{ old('contact_person_emergency', $user->contact_person_emergency) }}" required>
                        </div>

                        <div class="form-group">
                            <label class="form-label" for="contact_number_emergency">Emergency Contact Number</label>
                            <input type="text" id="contact_number_emergency" name="contact_number_emergency"
                                class="form-control"
                                value="{{ old('contact_number_emergency', $user->contact_number_emergency) }}" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-2">
                        <div class="form-group">
                    ...
                    <div class="responsive-grid-3">
                        <div class="form-group">
                            ...
                            <select id="lib_position_id" name="lib_position_id" class="form-control" required>
                                ...
                                @foreach($positions as $position)
                                    <option value="{{ $position->id }}" {{ old('lib_position_id', $user->lib_position_id) == $position->id ? 'selected' : '' }}>{{ $position->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="lib_region_id" name="lib_region_id" class="form-control" onchange="filterClubs()"
                                required>
                        ...
                        <div class="form-group">
                            ...
                            <select id="lib_club_name_id" name="lib_club_name_id" class="form-control" required disabled>
                                ...
                                @foreach($clubs as $club)
                                    <option value="{{ $club->id }}" data-region-id="{{ $club->lib_region_id }}" {{ old('lib_club_name_id', $user->lib_club_name_id) == $club->id ? 'selected' : '' }}>
                                        {{ $club->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group" style="margin-top: 1.5rem; display: flex; align-items: center; gap: 0.75rem;">
                        <input type="checkbox" id="make_private" name="make_private" value="1" {{ old('make_private', $user->make_private) ? 'checked' : '' }}
                            style="width: 1.25rem; height: 1.25rem; accent-color: var(--accent);">
                        <label class="form-label" for="make_private"
                            style="margin-bottom: 0; cursor: pointer; color: var(--text-main); font-weight: 600;">Keep my
                            profile private (Hide from public search and mapping)</label>
                    </div>

                    <div class="mt-6">
                        <button type="submit" class="btn btn-primary" style="width: 100%;">Save Profile and
                            Continue</button>
                    </div>
                </form>
            </div>
