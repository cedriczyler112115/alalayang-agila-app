...
@section('content')
    ...
    <div style="margin-top: 2rem; margin-bottom: 3rem;">
        ...
        <!-- Main Profile Card -->
        <div class="card"
            style="border-radius: var(--radius-lg); border: 1px solid var(--border-color); overflow: hidden; box-shadow: var(--shadow-lg);">

            <!-- Header Banner -->
            <div
                style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #3b82f6 100%); padding: 2.5rem 2rem 2rem 2rem; color: #ffffff; position: relative;">
                <div style="display: flex; align-items: center; gap: 1.75rem; flex-wrap: wrap;">
                    ...
                    @endphp
                    <img src="{{ $photoUrl }}" alt="{{ $user->fullname }}" title="Click to view full image"
                    ...
                    <div style="flex: 1; min-width: 240px;">
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 0.35rem; flex-wrap: wrap;">
                            <h1
                                style="font-size: 1.8rem; font-weight: 800; margin: 0; color: #ffffff; letter-spacing: -0.02em;">
                                Kuya {{ $user->fullname }}
                            </h1>
                            @if($user->status === 1)
                                <span
                                    style="background: rgba(34, 197, 94, 0.2); border: 1px solid rgba(34, 197, 94, 0.4); color: #4ade80; padding: 2px 10px; border-radius: 9999px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">Active
                                    KUYA</span>
                            @endif
                        </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr; gap: 2rem;" id="profile-grid">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                        ...
                        <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem;">
                            <h3
                                style="font-size: 1.1rem; font-weight: 700; margin: 0; color: var(--text-main); text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 8px;">
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <span
                                    style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ $user->sex ?? 'N/A' }}</span>
                            </div>

                            <div>
                                <span
                                    style="display: block; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 2px;">Marital
                                    Status</span>
                                <span
                                    style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ $user->marital_status ?? 'N/A' }}</span>
                            </div>

                            <div>
                                <span
                                    style="display: block; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 2px;">Occupation
                                    / Job</span>
                                <span
                                    style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ $user->current_job ?? 'N/A' }}</span>
                            </div>

                            <div>
                                <span
                                    style="display: block; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 2px;">Office
                                    / Company</span>
                                <span
                                    style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ $user->office ?? 'N/A' }}</span>
                            </div>
                        </div>

                        <div>
                            <span
                                style="display: block; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 2px;">Address</span>
                            <span
                                style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ $user->address ?? 'No address listed' }}</span>
                        </div>
                        ...
                        @if($user->eagle_id_card)
                            <div style="margin-top: 1rem;">
                                <span
                                    style="display: block; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); text-transform: uppercase; margin-bottom: 0.5rem;">Eagle
                                    Identification Card</span>
                                @php
                                    $eagleUrl = asset('storage/' . $user->eagle_id_card);
                                @endphp
                                <img src="{{ $eagleUrl }}" alt="Eagle ID Card" title="Click to view full image"
                                    ...
                                    onmouseout="this.style.transform='scale(1)';" onerror="this.style.display='none'">
                            </div>
                        @endif
                    ...
                    @if($user->location && str_contains($user->location, ','))
                        <div style="display: flex; flex-direction: column; gap: 1rem;">
                            ...
                            <div id="memberMap"
                                style="height: 350px; width: 100%; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                            </div>
                        </div>
                    @endif
    ...
    <div id="imagePreviewModal" onclick="closeImagePopup(event)"
        ...
        <div style="position: relative; max-width: 90vw; max-height: 90vh; display: flex; flex-direction: column; align-items: center; background: var(--card-bg); padding: 1rem; border-radius: var(--radius-lg); border: 1px solid var(--border-color); box-shadow: var(--shadow-lg);"
            onclick="event.stopPropagation();">
            <div
                style="width: 100%; display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; color: var(--text-main);">
